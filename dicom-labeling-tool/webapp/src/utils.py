...
#     if is_zip_oversized(temp_zip_file):
#         st.warning('Oversized zip file.')
#         clear_data_storage(temp_zip_file)
#         return jsonify({"message": "Oversized zip file"}), 400

#     with zipfile.ZipFile(temp_zip_file) as zip_ref:
#         zip_ref.extractall(temp_data_directory + request.json['session_id'] + '/')
...
# @app.route('/download_zip_from_url', methods=['POST'])
...
#         st.warning('Loading data from the web.')
#         gdd.download_file_from_google_drive(file_id=file_id,
#                                             dest_path=temp_zip_file,
#                                             overwrite=True)

#         if does_zip_have_dcm(temp_zip_file):
...
#             return jsonify({"message": "Zip file downloaded and extracted successfully"})

#     else:
#         st.warning('Not valid URL.')
#         return jsonify({"message": "Not a valid URL"}), 400

#     return jsonify({"message": "Failed to download zip file"}), 400


# @app.route('/processing_data', methods=['POST'])
# def processing_data():
#     path = request.json['path']
#     img3d, df = processing_data(path)
#     # Your processing logic here
#     return jsonify({"message": "Data processed successfully"})


# @app.route('/get_series_names', methods=['POST'])
# def get_series_names():
#     folder_names = request.json['folder_names']
#     series_names = get_series_names(folder_names)
#     return jsonify({"series_names": series_names})


# @app.route('/display_info', methods=['POST'])
# def display_info():
#     path = request.json['path']
#     df = display_info(path)
#     return jsonify({"info": df.to_dict()})


# @app.route('/read_DICOM_slices', methods=['POST'])
# def read_DICOM_slices():
#     path = request.json['path']
#     img3d, df = read_DICOM_slices(path)
#     # Your processing logic here
#     return jsonify({"message": "DICOM slices read successfully"})
...
#     filtered_img = filter_image(threshold_value, img)
#     # Your filtering logic here
#     return jsonify({"message": "Image filtered successfully"})
...
#     return jsonify({"message": "Image normalized successfully"})


# @app.route('/download_button', methods=['POST'])
# def download_button():
#     object_to_download = request.json['object_to_download']
#     download_filename = request.json['download_filename']
#     button_text = request.json['button_text']
#     pickle_it = request.json['pickle_it']
#     dl_link = download_button(object_to_download, download_filename, button_text, pickle_it)
#     return jsonify({"download_link": dl_link})


# @app.route('/clear_data_storage', methods=['POST'])
# def clear_data_storage():
#     path = request.json['path']
#     clear_data_storage(path)
#     return jsonify({"message": "Data storage cleared successfully"})
...
# def display_info(path):
#     columns = ['PatientID', 'PatientName', 'StudyDescription', 'PatientBirthDate', 'StudyDate', 'Modality',
#                'Manufacturer', 'InstitutionName', 'ProtocolName']
#     col_dict = {col: [] for col in columns}
#     dicom_object = pydicom.dcmread(path + os.listdir(path)[0])
...
#     return f'<a download="{download_filename}" href="data:file/txt;base64,{b64}">{button_text}</a><br></br>'
